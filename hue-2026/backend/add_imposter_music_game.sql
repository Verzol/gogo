...
begin
  ...
  return jsonb_build_object(
    ...
    'myTrack', case when v_track.id is null then null else jsonb_build_object('id', v_track.id, 'youtubeUrl', v_track.youtube_url, 'startSeconds', v_track.start_seconds, 'durationSeconds', v_track.duration_seconds) end,
...
begin
  ...
  update public.imposter_music_room set round = round + 1, status = 'prepared', common_track_id = p_common_track_id, imposter_track_id = p_imposter_track_id, starts_at = null, updated_at = now() where singleton;
  ...
  delete from public.imposter_music_ready;
  return public.imposter_music_get_state(p_session_token);
end;
$$;
...
begin
  perform public.imposter_music_is_host(p_session_token);
  update public.imposter_music_room set status = 'finished', starts_at = null, updated_at = now() where singleton;
  return public.imposter_music_get_state(p_session_token);
end;
$$;
